...
-- Mỗi đợt khoán có thể gán hệ số riêng cho từng dự án,
-- dùng để điều chỉnh KPI nhân sự khi phân chia quỹ khoán.

CREATE TABLE IF NOT EXISTS period_project_factors (
...
CREATE INDEX IF NOT EXISTS idx_ppf_period ON period_project_factors(period_id);

-- Cập nhật fn_allocate_smart: dùng period_project_factors thay vì project_dept_factors
...
BEGIN
  ...
  -- Tính KPI cho từng NV: group tasks → weighted score → nhân hệ số khó
  FOR r IN
    SELECT
      ...
      -- Hệ số khó: lấy từ period_project_factors theo đợt khoán hiện tại
      CASE
