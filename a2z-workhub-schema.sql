...
CREATE OR REPLACE FUNCTION public.user_org_id() RETURNS UUID AS $$ SELECT org_id FROM users WHERE id=auth.uid(); $$ LANGUAGE SQL SECURITY DEFINER STABLE;
CREATE OR REPLACE FUNCTION public.user_dept_id() RETURNS UUID AS $$ SELECT dept_id FROM users WHERE id=auth.uid(); $$ LANGUAGE SQL SECURITY DEFINER STABLE;
CREATE OR REPLACE FUNCTION public.user_role() RETURNS user_role AS $$ SELECT role FROM users WHERE id=auth.uid(); $$ LANGUAGE SQL SECURITY DEFINER STABLE;
...
-- Tasks (role-based)
CREATE POLICY "t_leader" ON tasks FOR ALL USING (org_id=public.user_org_id() AND public.user_role() IN ('admin','leader'));
...
CREATE POLICY "t_staff_r" ON tasks FOR SELECT USING (org_id=public.user_org_id() AND public.user_role()='staff' AND assignee_id=auth.uid());
CREATE POLICY "t_staff_u" ON tasks FOR UPDATE USING (org_id=public.user_org_id() AND public.user_role()='staff' AND assignee_id=auth.uid());
...
-- Audit
CREATE POLICY "audit_r" ON audit_logs FOR SELECT USING (org_id=public.user_org_id() AND public.user_role() IN ('admin','leader'));
...
CREATE POLICY "set_m" ON org_settings FOR ALL USING (org_id=public.user_org_id() AND public.user_role() IN ('admin','leader'));

-- Workflows
CREATE POLICY "wf_r" ON workflow_templates FOR SELECT USING (org_id=public.user_org_id());
CREATE POLICY "wf_m" ON workflow_templates FOR ALL USING (org_id=public.user_org_id() AND public.user_role() IN ('admin','leader'));

-- Dashboards
CREATE POLICY "dash_own" ON dashboards FOR ALL USING (owner_id=auth.uid());
CREATE POLICY "dash_shared" ON dashboards FOR SELECT USING (org_id=public.user_org_id() AND is_shared=TRUE);
