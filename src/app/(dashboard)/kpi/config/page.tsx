...
export default function KPIConfigPage() {
  ...
  const centerId = selectedCenter || null;

  const { data: config } = useAllocationConfig(centerId);

  return (
    <div className="max-w-xl space-y-5 animate-fade-in">
      <div className="flex items-center gap-3">
        <div className="w-56">
          <label className="text-xs text-muted-foreground font-medium">Phạm vi áp dụng</label>
          <SearchSelect value={selectedCenter} onChange={setSelectedCenter}
            options={[
              { value: "", label: "Toàn công ty (mặc định)" },
              ...centers.map((c: any) => ({ value: c.id, label: `${c.code || ""} — ${c.name}` })),
            ]}
            className="mt-1" />
        </div>
      </div>

      <WeightSection config={config} centerId={centerId} canEdit={!!canEdit} />
...
function WeightSection({ config, centerId, canEdit }: {
  config: any; centerId: string | null; canEdit: boolean;
}) {
  const { t } = useI18n();
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: typeof form) => {
      if (config?.id) {
        ...
        const { error } = await supabase.from("allocation_configs").insert({
          ...
          name: centerId ? `Config ${centerId}` : "Default Config",
          center_id: centerId,
          ...values,
        } as any);
        if (error) throw error;
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: kpiKeys.config(centerId) });
  ...
  return (
    <Section title={config ? t.kpi.currentWeights : t.kpi.createConfig}>
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-border">
          ...
          <span className={`font-mono text-base font-bold ${total >= 100 ? "text-green-500" : "text-amber-500"}`}>
            {total}%
          </span>
        </div>
        {canEdit && (
